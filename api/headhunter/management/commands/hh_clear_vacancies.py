...
class Command(BaseCommand):
    ...
    def _print_summary(self, summary: Dict[str, Any], ctx: Dict[str, Any]):
        if ctx['quiet']:
            return
    ...
    def _print_dry_run(self, queryset, summary: Dict[str, Any], ctx: Dict[str, Any]):
        if not ctx['quiet']:
    ...
    def _confirm(self, vacancy_count: int) -> bool:
        ...
        return confirm.lower() in ['yes', 'y', 'да']

    def _perform_deletion(self, queryset, summary: Dict[str, Any], ctx: Dict[str, Any], filters_applied: List[str]):
        try:
            with transaction.atomic():  # type: ignore[misc]
                ...
                if remaining_vacancies > 0 and not ctx['json_output']:
                    self.stdout.write(f'ВНИМАНИЕ: Осталось {remaining_vacancies} вакансий, которые не были удалены.')
                    remaining_ids = list(VacancyModel.objects.filter(id__in=vacancy_ids).values_list('id', flat=True))
                    if remaining_ids:
            ...
            self._out(f'Ошибка при удалении: {e}', ctx)
            logger.exception('Ошибка при очистке вакансий')
            raise

    def _reset_sequences(self, ctx: Dict[str, Any]):
    ...
    def _out(self, message: str, ctx: Dict[str, Any]):
        if ctx['json_output'] or ctx['quiet']:
            return
